...
class Shell(Cmd):
    ...
    def do_keylist(self, arg):
        ...
        for k in self.keys:
            ...
            if 'pub' in k.keys():
                print('Public Key:')
                print('\tX: {0} ({1} bits)'.format(str(k['pub'][0]), k['pub'][0].bit_length()))
                print('\tY: {0} ({1} bits)'.format(str(k['pub'][1]), k['pub'][1].bit_length()))
            if 'priv' in k.keys():
                print('Private Key:')
                print('\t{0} ({1} bits)'.format(str(k['priv']), k['priv'].bit_length()))
            if 'curve' in k.keys():
                print('Curve:')
                for name, p in zip(curve_params_sequence, k['curve']):
                    print('\t{0}: {1} ({2} bits)'.format(name, str(p), p.bit_length()))
            print('\n')
            cntr += 1
    ...
    @_pubkey_warning
    def do_verify(self, path_to_file, sign, pub_key):
        """
        ...
        return verify_file(self.key['curve'], path_to_file, sign, own_pubkey=pub_key)
        # try:
        #     if len(paths) == 2:
